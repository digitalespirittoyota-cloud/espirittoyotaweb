...
interface VariantSpecs {
  ...
  COMFORT?: SpecSection;
  FUEL?: SpecSection;
  DIMENSIONS_WEIGHT?: SpecSection;
  CHASSIS?: SpecSection;
  SUSPENSION_BRAKES?: SpecSection;
  "WHEEL & TYRE SIZE"?: SpecSection;
}


/* ===================== SPEC DATA ===================== */
/* ===================== SPEC DATA ===================== */
const specifications: Record<Powertrain, Partial<Record<Variant, VariantSpecs>>> = {
  HYBRID: {
    "HYBRID-V": {
      ENGINE: {
        title: "ENGINE",
            ...
            Object.values(currentSpecs)
              ...
              .map((section, i) =>
                section?.data?.length > 0 ? (
                  <div key={i}>
                    <div className="spec-title">{section.title}</div>
                    {section.data.map((row: SpecItem, j: number) => (
                      <div className="spec-row" key={j}>
                        <span>{row.label}</span>
                        <span>
